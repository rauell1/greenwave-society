...
export default function CampaignCreator({ events = [] }: { events?: { id: string; title: string; slug: string }[] }) {
  ...
  const [form, setForm] = useState({
    ...
    subject: "",
    preheader: "",
    eyebrow: "Greenwave Update",
    htmlBody: "<p>Hello Greenwave community,</p><p>Share your update here.</p>",
    audience: "newsletter"
  });
  ...
  async function create() {
    ...
    setBusy(false);
    if (response.ok) {
      router.push(`/admin/communications/${data.campaign.id}`);
      router.refresh();
    } else {
  ...
  return (
    <section className="rounded-xl border bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 grid gap-3">
        ...
        <button disabled={busy || !form.name || !form.subject} onClick={() => void create()} className="rounded-lg bg-emerald-700 px-4 py-2 text-white disabled:opacity-50 w-full sm:w-auto mt-2">
          Create draft
        </button>
