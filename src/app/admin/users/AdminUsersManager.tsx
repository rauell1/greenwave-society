...
export default function AdminUsersManager({ currentUserId, canManageAccess }: { currentUserId: string; canManageAccess: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSelectedId(current => data.users.some((user: User) => user.id === current) ? current : data.users[0]?.id ?? null);
  ...
  async function remove() {
    if (!deleteTarget || deleteEmail.trim().toLowerCase() !== deleteTarget.email.toLowerCase() || saving) return;
    setSaving(true); setMessage("");
    try {
      const response = await fetch(`/api/admin/users/${deleteTarget.id}`, { method: "DELETE" });
      const data = await response.json();
      if (!response.ok) { setMessage(data.error ?? "The administrator could not be deleted."); return; }
      setMessage("Administrator deleted. Their login access and reset links have been removed.");
      setDeleteTarget(null); setDeleteEmail("");
      await load();
    } catch { setMessage("The administrator could not be deleted. Please try again."); }
    finally { setSaving(false); }
  }

  return <div className="space-y-6">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">Administration</p><h1 className="mt-1 text-2xl font-semibold text-slate-950">Administrators</h1><p className="mt-1 text-sm text-slate-500">{canManageAccess ? "Add administrators and control their access by role." : "View administrator accounts, executive positions, and assigned access."}</p></div>{canManageAccess ? <button onClick={() => setShowAdd(value => !value)} className="rounded-lg bg-emerald-700 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-800">{showAdd ? "Cancel" : "Add administrator"}</button> : <span className="w-fit rounded-full bg-slate-100 px-3 py-1.5 text-xs font-medium text-slate-600">Read-only access</span>}</header>
    ...
    <section className="grid min-h-[480px] overflow-hidden rounded-xl border bg-white lg:grid-cols-[minmax(280px,0.75fr)_1.4fr]">
      ...
      <div className="p-5 sm:p-6">{selected ? <div className="mx-auto max-w-2xl"><div className="flex flex-col gap-4 border-b pb-5 sm:flex-row sm:items-start sm:justify-between"><div><p className="break-all font-semibold text-slate-950">{selected.email}</p><p className="mt-1 text-xs text-slate-500">{selected.id === currentUserId ? "This is your account. Your own access cannot be changed here." : canManageAccess ? "Choose the roles this administrator needs." : "Account and access details are read-only."}</p></div><label className={`flex items-center gap-2 text-sm font-medium ${!canManageAccess || selected.id === currentUserId ? "text-slate-400" : "text-slate-700"}`}><input type="checkbox" checked={selected.isActive} disabled={!canManageAccess || selected.id === currentUserId} onChange={event => setUsers(values => values.map(user => user.id === selected.id ? { ...user, isActive: event.target.checked } : user))} className="h-4 w-4 accent-emerald-700" />Active account</label></div>
        ...
        {canManageAccess && selected.id !== currentUserId ? <div className="mt-6 flex flex-wrap items-center justify-between gap-4 border-t pt-5"><p className="text-xs text-slate-500">Disabling an account immediately revokes its sessions.</p><button onClick={() => void save(selected)} disabled={saving || selected.roles.length === 0} className="shrink-0 rounded-lg bg-slate-950 px-4 py-2.5 text-sm font-medium text-white disabled:opacity-40">{saving ? "Saving…" : "Save access"}</button><button type="button" disabled={saving} onClick={() => { setDeleteTarget(selected); setDeleteEmail(""); setMessage(""); }} className="shrink-0 rounded-lg border border-red-300 px-4 py-2.5 text-sm font-medium text-red-700 hover:bg-red-50 disabled:opacity-40">Delete administrator</button></div> : null}</div> : <div className="flex h-full min-h-64 items-center justify-center text-sm text-slate-500">Select an administrator to view access.</div>}</div>
    </section>
    {deleteTarget ? <section aria-labelledby="delete-admin-title" className="rounded-xl border border-red-200 bg-red-50 p-5">
      <h2 id="delete-admin-title" className="font-semibold text-red-900">Delete administrator</h2>
      <p className="mt-2 text-sm text-red-900">Deleting <strong className="break-all">{deleteTarget.email}</strong> removes their admin access, password, and reset links. Existing sessions are signed out. Past content and audit history are kept.</p>
      <p className="mt-2 text-sm text-red-900">To give them access again, you will need to add their email and roles again.</p>
      <label htmlFor="delete-admin-email" className="mt-4 block text-sm font-medium text-red-900">Type their email to confirm</label>
      <input id="delete-admin-email" type="email" autoComplete="off" value={deleteEmail} onChange={event => setDeleteEmail(event.target.value)} className="mt-1 w-full rounded-lg border border-red-300 bg-white px-3 py-2 text-sm" />
      <div className="mt-4 flex gap-3"><button type="button" disabled={saving || deleteEmail.trim().toLowerCase() !== deleteTarget.email.toLowerCase()} onClick={() => void remove()} className="rounded-lg bg-red-700 px-4 py-2 text-sm font-medium text-white disabled:opacity-40">{saving ? "Deleting…" : "Confirm deletion"}</button><button type="button" disabled={saving} onClick={() => { setDeleteTarget(null); setDeleteEmail(""); }} className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm">Cancel</button></div>
    </section> : null}
  </div>;
}
