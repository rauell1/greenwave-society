...
export default function ContentManager({ capabilities }: { capabilities: Capabilities }) {
  ...
  return <div className="grid gap-6 xl:grid-cols-[360px_1fr]">
    <aside className="rounded-xl border border-emerald-100 bg-white shadow-sm">
      <div className="flex items-center justify-between border-b p-4"><div><h1 className="font-semibold text-slate-900">Content</h1><p className="text-xs text-slate-500">Drafts and published entries</p></div>{capabilities.create && <button className="rounded-lg bg-emerald-700 px-3 py-2 text-sm font-medium text-white" onClick={() => { setSelected(null); setForm(empty); setMessage(""); }}>New</button>}</div>
    ...
    <section className="rounded-xl border border-emerald-100 bg-white p-6 shadow-sm">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="text-sm font-medium text-slate-700">Type<select value={form.type} onChange={e => setForm({ ...form, type: e.target.value })} className="mt-1 w-full rounded-lg border p-2.5"><option value="page">Page</option><option value="program">Program</option><option value="story">Story</option><option value="announcement">Announcement</option></select></label>
